...
bool StatusLayer::init() {
	...
	Number::getInstance()->loadNumber(NUMBER_SCORE.c_str(),
			"number_score_%02d");
...
void StatusLayer::showReadyStatus() {
	scoreSprite = (Sprite *) Number::getInstance()->convert(NUMBER_FONT.c_str(),
			0);
...
void StatusLayer::onGamePlaying(int score) {
	this->removeChild(scoreSprite);
	this->scoreSprite = (Sprite*) Number::getInstance()->convert(
			NUMBER_FONT.c_str(), score);
	scoreSprite->setPosition(
			Point(this->originPoint.x + this->visibleSize.width / 2,
					this->originPoint.y + this->visibleSize.height * 5 / 6));
	this->addChild(scoreSprite);
...
void StatusLayer::blinkFullScreen() {
	...
	CallFunc *actionDone = CallFunc::create(
			bind(&StatusLayer::fadeInGameOver, this));
...
void StatusLayer::fadeInGameOver() {
	...
	// Start next action
	CallFunc *actionDone = CallFunc::create(
			bind(&StatusLayer::jumpToScorePanel, this));
...
void StatusLayer::jumpToScorePanel() {
	...
	auto bestScoreSprite = (Sprite *) Number::getInstance()->convert(
			NUMBER_SCORE.c_str(), this->bestScore, Gravity::GRAVITY_RIGHT);
...
void StatusLayer::fadeInRestartBtn() {
	...
	menu->setPosition(
			Point(
					this->originPoint.x + this->visibleSize.width / 2
							- restartBtn->getContentSize().width / 2,
					this->originPoint.y + this->visibleSize.height * 2 / 7
							- 10.0f));
	tmpNode->addChild(menu);

	//create the rate button. however ,this button is not available yet = =
	Sprite* rateBtn = Sprite::createWithSpriteFrame(
			AtlasLoader::getInstance()->getSpriteFrameByName("button_score"));
	rateBtn->setPosition(
			Point(
					this->originPoint.x + this->visibleSize.width / 2
	...
	CallFunc *actionDone = CallFunc::create(
			bind(&StatusLayer::refreshScoreCallback, this));
...
void StatusLayer::refreshScoreExecutor(float dt) {
	...
	scoreSprite = (Sprite *) Number::getInstance()->convert(
			NUMBER_SCORE.c_str(), this->tmpScore, Gravity::GRAVITY_RIGHT);
	scoreSprite->setAnchorPoint(Point(1, 0));
	scoreSprite->setPosition(
			Point(this->originPoint.x + this->visibleSize.width * 3 / 4 + 20.0f,
					this->originPoint.y + this->visibleSize.height * 1 / 2));
...
void StatusLayer::setBlinkSprite() {
	...
	for (int i = 0; i < 3; i++) {
		const char *filename =
				String::createWithFormat("blink_%02d", i)->getCString();
		SpriteFrame *frame = AtlasLoader::getInstance()->getSpriteFrameByName(
				filename);
