...
bool GameLayer::init() {
	if (Layer::init()) {
		...
		this->bird->setPosition(origin.x + visiableSize.width * 1 / 3 - 5,
				origin.y + visiableSize.height / 2 + 5);
		this->bird->idle();
		...
		this->landSpite2->setPosition(
				this->landSpite1->getContentSize().width - 2.0f, 0);
		...
		/******************* CODE NOT WORKING DO NOT REVEL THIS CODE********/
		contactListener->onContactBegin = CC_CALLBACK_1(
				GameLayer::onContactBegin,this);
...
void GameLayer::scrollLand(float dt) {
	...
	this->landSpite2->setPositionX(
			this->landSpite1->getPositionX()
					+ this->landSpite1->getContentSize().width - 2.0f);
...
void GameLayer::createPips() {
	...
	for (int i = 0; i < PIP_COUNT; i++) {
		...
		singlePip->setPosition(
				visibleSize.width + i * PIP_INTERVAL + WAIT_DISTANCE,
...
void GameLayer::gameOver() {
	...
	//get the best score
	int bestScore = UserRecord::getInstance()->readIntegerFromUserDefault(
			"best_score");
	//update the best score
	if (this->score > bestScore) {
		UserRecord::getInstance()->saveIntegerToUserDefault("best_score",
				this->score);
	}
...
void GameLayer::birdSpriteFadeOut() {
	...
	CallFunc* animationDone = CallFunc::create(
			bind(&GameLayer::birdSpriteRemove, this));
	Sequence* sequence = Sequence::createWithTwoActions(animation,
			animationDone);
